...
syntax_tree::syntax_tree(struct syn_tree *tree) {
  if (tree == nullptr) {
    ...
    deleteSynTree(tree);
    root = std::shared_ptr<syntax_program>(static_cast<syntax_program *>(node));
  }
}
...
struct syntax_tree_node *syntax_tree::transform_node_iter(struct tree_node *n) {
  if (STR_EQ(n->name, "program")) {
    ...
    while (!s.empty()) {
      ...
      auto child_node_shared = std::shared_ptr<syntax_declaration>(child_node);
    ...
    if (list_ptr->child_num != 0) {   // is this param-list?
      ...
      while (!s.empty()) {
        ...
        auto child_node_shared = std::shared_ptr<syntax_param>(child_node);
    ...
    if (n->children[1]->child_num == 2) { // local -> local var
      ...
      while (!s.empty()) {
        auto decl_node =
            static_cast<syntax_var_declaration *>(transform_node_iter(s.top()));
    ...
    if (n->children[2]->child_num == 2) {
      ...
      while (!s.empty()) {
        auto stmt_node =
            static_cast<syntax_statement *>(transform_node_iter(s.top()));
    ...
    if (n->child_num == 2) {
      auto expr_node =
          static_cast<syntax_expression *>(transform_node_iter(n->children[0]));
    ...
    auto expr_node =
        static_cast<syntax_expression *>(transform_node_iter(n->children[2]));
    auto expr_node_ptr = std::shared_ptr<syntax_expression>(expr_node);
    node->expression = expr_node_ptr;

    auto if_stmt_node =
        static_cast<syntax_statement *>(transform_node_iter(n->children[4]));
    auto if_stmt_node_ptr = std::shared_ptr<syntax_statement>(if_stmt_node);
    node->if_statement = if_stmt_node_ptr;

    if (n->child_num == 7) {
      auto else_stmt_node =
          static_cast<syntax_statement *>(transform_node_iter(n->children[6]));
    ...
    if (n->child_num == 3) {
      auto expr_node =
          static_cast<syntax_expression *>(transform_node_iter(n->children[1]));
      auto expr_node_ptr = std::shared_ptr<syntax_expression>(expr_node);
      node->expression = expr_node_ptr;
    }
    ...
    if (n->child_num == 3) {
      ...
      auto factor_node =
          static_cast<syntax_factor *>(transform_node_iter(n->children[2]));
      auto factor_node_ptr = std::shared_ptr<syntax_factor>(factor_node);
      node->factor = factor_node_ptr;
    } else {
      auto factor_node =
          static_cast<syntax_factor *>(transform_node_iter(n->children[0]));
      auto factor_node_ptr = std::shared_ptr<syntax_factor>(factor_node);
      node->factor = factor_node_ptr;
    ...
    if (STR_EQ(n->children[2]->children[0]->name, "arg-list")) {
      ...
      while (!s.empty()) {
        auto expr_node =
            static_cast<syntax_expression *>(transform_node_iter(s.top()));
        auto expr_node_ptr = std::shared_ptr<syntax_expression>(expr_node);
